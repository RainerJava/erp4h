 ...
 *
 */
public abstract class FLogFrame extends InspectFrame {

	/** */
	private static final long serialVersionUID = 1L;
	/** */
	static Logger logger = Logger.getLogger(FLogFrame.class);
	/** */
	 ...
	 * </DL>
	 */
	public FLogFrame(BaseFrame frame, String title) {
        super(title);
        parentFrame = frame;
        init();
	}
	 ...
	 * </DL>
	 */
	public FLogFrame(BaseFrame frame) {
        super("");
        parentFrame = frame;
        init();
	}
    ...
    private void init() {
        ...
        ExitMasterWindow exitWin = new ExitMasterWindow(btnF11);
...
//	  	m_table.setLocation(1, 0);
				...
				if (rVal == JFileChooser.APPROVE_OPTION) 
				{
					...
					try
					{
						...
						for(int i = 0; i < iCount;i++)
						{
							...
							{
								if(i == 0){
					    			Label lbHeader = new Label(j, i,arr.get(j) , headFormat);	
					    			s1.addCell(lbHeader);
					    			s1.setColumnView(i, 20); 
								}else {
									try
									{
										Label l1 = new Label(j, i, arr.get(j),leftCellFormat);
										s1.addCell(l1);
									}
	...
	public void stopTimerRemoveListener(JFrame frame) {
		if (timer.isRunning()) {
			timer.stop();
			timer = null;
		}
