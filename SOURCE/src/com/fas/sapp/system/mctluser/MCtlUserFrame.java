 ...
 * </DL>
 */
public class MCtlUserFrame extends BaseMasterFrame {

	/** */
	private static final long serialVersionUID = 1L;
	/** Log */
	static Logger logger = Logger.getLogger(MCtlUserFrame.class);
	...
	* </DL>
	*/
	public MCtlUserFrame(BaseFrame frame, String title) {
		super(frame, title);
		init();
	}
	...
	* </DL>
	*/
	public MCtlUserFrame(BaseFrame frame) {
		super(frame);
		init();
	}
	...
	*/
	...
	{		
		...
		try {			
			...
			//Condition: USERID=current user, C_KEY from cbb,
			//MTN_FLG=0
	        //Function load table data			
			String strUserid = ApplicationConstants.getLoginUser().getUserId();
...
//			list.add(lstData.get(i).getCName()); // C_NAME
//			list.add(lstData.get(i).getCData()); // C_DATA
//			list.add(lstData.get(i).getCKey()); // C_KEY
//			
...
//		});
	...
	{	
		...
		//Check and fill Data to Current User from User SYSTEM
		mCtlService = new MCtlServiceImpl();
		try {			
			String strFromUserid = mCtlService.getByKey("SYSTEM", "Z_SYSUSR").getCData();
			String strToUserid = ApplicationConstants.getLoginUser().getUserId();
			int iMtnFlg = 0;			
			mCtlService.fillDataFromUseridToUserid(strFromUserid, strToUserid, iMtnFlg);
	...
	protected void doF8() {	
		...
		if (iSelectedRow >= 0 && m_table.getRowCount() > 0) 
		{
			...
			if (dataVo != null) {
				...
				MCtlUserFrameEdit a =  new MCtlUserFrameEdit(this.getFrame(), true, strUserid, strCKey, bPermission);
				a.pack();
				//a.doF11();
				a.setVisible(true);
				doF10();
...
//				lblPermission.setText(NameConstants.getName("USEFLG"+"1"));
//				lblPermission.setBorder(null);
//			}		
	}
