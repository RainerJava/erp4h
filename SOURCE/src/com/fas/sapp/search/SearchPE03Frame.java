 ...
 * @description
 */
public class SearchPE03Frame extends SearchTransactionFrame {
	 ...
	 * </DL>
	 */
	public SearchPE03Frame(BaseFrame frame) {
		super(frame, true);
        init();
	}
	 ...
	 * @param title
	 */
	public SearchPE03Frame(BaseFrame frame, String title) {
        super(frame, true);
        setTitle(title);
        init();
	}
	 ...
	 * @param modal
	 */
	public SearchPE03Frame(BaseFrame frame, boolean modal) {
        super(frame, modal);
        init();
	}
	 ...
	 * </DL>
	 */
	public SearchPE03Frame(BaseFrame frame,SortObjVo sortVo) {
	...
	protected TrSchdspVo getSchDspVo() {
		...
		dspVo.setDspCode(StringUtils.convertIntegerToStr(SearchTransactionFrame.I_PE03_SEARCH));			
		dspVo.setDspName("売上先検索");			
		dspVo.setSchCtl("SCH_PE03");
		dspVo.setTblName("PE03");
		
		// SQL_ID: SQLID get from XML
		dspVo.setSqlId("PE03001");			
...
//		m_table.getTableHeader().setPreferredSize(new Dimension(m_table.getTableHeader().getWidth(),FaceContants.TABLE_HEADER_HEIGHT));
	...
	public static void main(String args[]) {
		//Use for testing
		SearchPE03Frame templte = new SearchPE03Frame();
		templte.pack();
