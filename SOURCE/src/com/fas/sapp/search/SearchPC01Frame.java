 ...
 * @description
 */
public class SearchPC01Frame extends SearchTransactionFrame {
	 ...
	 * </DL>
	 */
	public SearchPC01Frame(BaseFrame frame) {
		super(frame, true);
        init();
	}
	 ...
	 * @param title
	 */
	public SearchPC01Frame(BaseFrame frame, String title) {
        super(frame, true);
        setTitle(title);
        init();
	}
	 ...
	 * @param modal
	 */
	public SearchPC01Frame(BaseFrame frame, boolean modal) {
        super(frame, modal);
        init();
	}
	 ...
	 * </DL>
	 */
	public SearchPC01Frame(BaseFrame frame,SortObjVo sortVo) {
	...
	protected TrSchdspVo getSchDspVo() {
		...
		dspVo.setDspCode(StringUtils.convertIntegerToStr(SearchTransactionFrame.I_PC01_SEARCH));			
		dspVo.setDspName("承認番号検索");			
		dspVo.setSchCtl("SCH_PC01");
		dspVo.setTblName("PC01");
		
		// SQL_ID: SQLID get from XML
		dspVo.setSqlId("PC01001");			
		// TTL_NAME1: Line 1 Label 1 
		dspVo.setTtlName1("所属支部");
		// TTL_NAME2: Line 2 Label 1 
		dspVo.setTtlName2("承認番号");
		...
		dspVo.setSpdColnm1("承認番号");
		dspVo.setSpdColnm2("所属支部");
...
//		//Use for testing
//		SearchPC01Frame templte = new SearchPC01Frame();
//		templte.pack();
//		templte.setVisible(true);
		
		for(int i=1; i<=91; i++)
		{
			System.out.println(", M.C01"+i);
		}
	}
